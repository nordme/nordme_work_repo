...
def plot_ventral(band):
    title = "%s (%d-%d Hz)" % ((band.capitalize(),) + freq_bands[band])
    topos[band].plot_topomap(
        times=0., scalings=1., cbar_fmt='%0.1f', vmin=0, cmap='inferno',
        time_format=title)
    brain = stcs[band].plot(
        subject=subject, subjects_dir=subjects_dir, views='ven', hemi='both',
        time_label=title, title=title, colormap='inferno', smoothing_steps=25,
        clim=dict(kind='percent', lims=(70, 85, 99)))
...
###############################################################################
# Alpha:
fig_adors, brain_alpha_dorsal = plot_dorsal('alpha')
brain_alpha_dorsal.save_image('/brainstudio/MEG/genz/genz_proc/resting/%s/brain_alpha_dorsal.png' % subject)

fig_aven, brain_alpha_ventral = plot_ventral('alpha')
brain_alpha_ventral.save_image('/brainstudio/MEG/genz/genz_proc/resting/%s/brain_alpha_ventral.png' % subject)

###############################################################################
# Beta:
fig_bdors, brain_beta_dorsal = plot_dorsal('beta')
brain_beta_dorsal.save_image('/brainstudio/MEG/genz/genz_proc/resting/%s/brain_beta_dorsal.png' % subject)

fig_bven, brain_beta_ventral = plot_ventral('beta')
brain_beta_ventral.save_image('/brainstudio/MEG/genz/genz_proc/resting/%s/brain_beta_ventral.png' % subject)
