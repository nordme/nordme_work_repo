...
params.epochs_prefix = ['All', 'FRN', 'SPN']
params.get_projs_from = np.arange(6)
params.inv_names = ['%s_aud', '%s_vis']   # generate auditory and visual inverses
params.inv_runs = [np.arange(3, 6), np.arange(0, 3)]
params.pick_events_cov = pick_aud_cov_events   # Only use auditory events for the covariance calculation
